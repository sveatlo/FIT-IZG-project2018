...
void phong_onInit(int32_t width,int32_t height){
	...
	cpu_bufferData(phong.gpu, phong.vbo, sizeof(bunnyIndices), bunnyIndices);
	cpu_bufferData(phong.gpu, phong.ebo, sizeof(bunnyVertices), bunnyVertices);

    cpu_createVertexPullers(phong.gpu, 1, &phong.puller);
    cpu_setVertexPullerHead(phong.gpu, phong.puller, 0, phong.ebo, 0, sizeof(float)*6);
    cpu_enableVertexPullerHead(phong.gpu, phong.puller, 0);
    cpu_setVertexPullerHead(phong.gpu, phong.puller, 1, phong.ebo, sizeof(float)*3, sizeof(float)*6);
