...
void const* gpu_computeVertexAttributeDataPointer(
    ...
    return (void*)((uint8_t*)head->buffer + head->offset + gl_VertexID * head->stride);
}
...
void gpu_runPrimitiveAssembly(
    ...
    for (long unsigned int i = 0; i  < nofPrimitiveVertices; i++, primitive->nofUsedVertices++) {
...
void gpu_computeScreenSpaceBarycentrics(
    ...
    // x = b.x - a.x, y = b.y - a.y
    Vec2 v0 = { .data = {vertices[1].data[0] - vertices[0].data[0], vertices[1].data[1] - vertices[0].data[1]} };
    // x = c.x - a.x, y = c.y - a.y
    Vec2 v1 = { .data = {vertices[2].data[0] - vertices[0].data[0], vertices[2].data[1] - vertices[0].data[1]} };
    // x = p.x - a.x, y = p.y - a.y
    Vec2 v2 = { .data = {pixelCenter->data[0] - vertices[0].data[0], pixelCenter->data[1] - vertices[0].data[1]} };
